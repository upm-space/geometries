...
/*    toD3Bezier_old(controlPoints,delta_t){
    ...
    toD3Bezier(controlPoints,delta_t){
        ...
        let t , i;
        for(t = 0; t <= 1.0; t += delta_t){
            var x = 0, y = 0;
            for(i = 0; i <= n ; i++){
                x += this.sBin(n,i)*Math.pow(t,i)*Math.pow((1-t),(n-i))*controlPoints[i].x;
                y += this.sBin(n,i)*Math.pow(t,i)*Math.pow((1-t),(n-i))*controlPoints[i].y;
             }
            let point = new Point(x, y, 0);
            curvePoints.push(point.toJson2D());
        }
        return curvePoints;
    }
    ...
    sBin(n, i)
    {
        let rvalbin = (this.sFact(n))/(this.sFact(i)*this.sFact(n-i));
        return rvalbin;
    }
